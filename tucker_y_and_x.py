...
LastEditTime: 2026-01-06 21:59:34
...
if __name__ == '__main__':
    ...
    # bad_y_ratio_list = [ 0.03,0.05,0.07,0.1,0.15]
    # percentile_list = [0]
            ...
            for model in models_list:
                while True:
                    try:
                        ...
                        # 如果运行到这里没有报错，说明成功，跳出 while 循环
                        break

                    except Exception as e:
                        # 捕获所有异常，打印错误信息，然后循环会继续（重试）
                        print(f"\n[Warning] {model.name} 运行出错: {e}")
                        print(f"正在重新运行 {model.name} ...\n")
                        # 稍微暂停一下，防止在某些硬件错误下死循环占用资源过高
                        time.sleep(1)
    # ==========================================
    # 打印最终表格 (Min RPE)
    # ==========================================
