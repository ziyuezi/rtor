...
def acc_fnr_fpr_from_masks(pred: np.ndarray, true: np.ndarray, eps: float = 1e-12):
    """
    pred/true: bool array, shape (n_samples,)
    Return: acc, fnr, fpr, tp, fp, tn, fn
    """
            ...
            for model in models_to_run:
                while True:
                    try:
                        ...
                        # =========================
                        # Outlier detection report (Sx & Sy)
                        # =========================

                        # ---- sample-level true labels (来自 idx_bad_*_train) ----
                        bad_x_true = np.zeros(model.Sx.shape[0], dtype=bool)
                        bad_x_true[params['idx_bad_x_train']] = True

                        bad_y_true = np.zeros(model.Sy.shape[0], dtype=bool)
                        bad_y_true[params['idx_bad_y_train']] = True

                        # ---- sample-level predicted labels (从 Sx/Sy 推断) ----
                        sx_nnz_ratio = (np.abs(model.Sx) > 1e-6).reshape(model.Sx.shape[0], -1).mean(axis=1)
                        sy_nnz_ratio = (np.abs(model.Sy) > 1e-6).reshape(model.Sy.shape[0], -1).mean(axis=1)

                        thr_sx = 0.01  # 1%：真实注入块占比约10%，因此阈值保守
                        thr_sy = 0.01

                        pred_bad_x = sx_nnz_ratio > thr_sx
                        pred_bad_y = sy_nnz_ratio > thr_sy

                        # ---- report Sx sample-level ACC/FNR/FPR ----
                        acc, fnr, fpr, tp, fp, tn, fn = acc_fnr_fpr_from_masks(pred_bad_x, bad_x_true)
                        print(
                            f"[{model.name}] Sx sample-level: ACC={acc:.4f} FNR={fnr:.4f} FPR={fpr:.4f} | TP={tp} FP={fp} TN={tn} FN={fn} | thr={thr_sx}")

                        # ---- report Sy sample-level ACC/FNR/FPR ----
                        acc, fnr, fpr, tp, fp, tn, fn = acc_fnr_fpr_from_masks(pred_bad_y, bad_y_true)
                        print(
                            f"[{model.name}] Sy sample-level: ACC={acc:.4f} FNR={fnr:.4f} FPR={fpr:.4f} | TP={tp} FP={fp} TN={tn} FN={fn} | thr={thr_sy}")
